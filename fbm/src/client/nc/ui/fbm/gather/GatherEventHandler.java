...
public class GatherEventHandler extends FBManageEventHandler {
	...
	//add lsl 201-08-31
	private GatherUI getGatherUI(){
		return (GatherUI)getBillUI();
	}
...
//		super.onBoQuery();
		...
		//add lsl 修改查询条件
		if(getBillUI()._getCorp().getPk_corp().equals("0001")){
			
			if(!strWhere.toString().contains(RegisterVO.REGISTERSTATUS)){
				strWhere.append(" and pk_corp <> '0001' and fbm_register.registerstatus in ('on_gather','register','has_return')");
			}else{
				strWhere.append(" and pk_corp <> '0001' ");
			}
