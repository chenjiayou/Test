 ...
 *
 */
public class ReturnProcessor extends AbstractDataProcessor {
	...
	public void buildData(SuperVO vo) throws BusinessException {
		...
		billVO.setChildrenVO(new CircularlyAccessibleValueObject[]{returnBVO});
		
		

		
		//保存
//		ActionParamVO[] params = DefaultParamAdapter.changeReturn2Param(billVO,FbmActionConstant.SAVE);
//		setUpgrade(params);
//		FbmActionFactory.getInstance().createActionClass(FbmBusConstant.BILLTYPE_RETURN, FbmActionConstant.SAVE).doAction(params);
		BusiActionFactory.getInstance().createActionClass(FbmBusConstant.BILLTYPE_RETURN, FbmActionConstant.SAVE).doAction(billVO, FbmActionConstant.SAVE,false);

		//审核
//		params = DefaultParamAdapter.changeReturn2Param(billVO,FbmActionConstant.AUDIT);
//		setUpgrade(params);
//		FbmActionFactory.getInstance().createActionClass(FbmBusConstant.BILLTYPE_RETURN, FbmActionConstant.AUDIT).doAction(params);
		BusiActionFactory.getInstance().createActionClass(FbmBusConstant.BILLTYPE_RETURN, FbmActionConstant.AUDIT).doAction(billVO, FbmActionConstant.AUDIT,false);
