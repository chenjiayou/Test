 ...
 *
 */
public class CancelAuditInnerKeep<K extends HYBillVO,T extends StorageVO> extends ActionInnerKeep<K, T> {
	...
	protected String doCheck(BusiActionParamVO<T> param) throws BusinessException {
		...
		if(!(actionVO.getActioncode().equals(FbmActionConstant.AUDIT)
			...
			return nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000239")/* @res"票据"*/ + param.getBaseinfoVO().getFbmbillno() + nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000266")/* @res"的前一操作必须是审核内部存放单。"*/+ BusiMessageTranslator.translateAction(param);
		}
		return null;
	}
