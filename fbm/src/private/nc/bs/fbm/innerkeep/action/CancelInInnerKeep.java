...
public class CancelInInnerKeep<K extends HYBillVO,T extends StorageVO> extends ActionInnerKeep<K, T> {
	...
	protected String doCheck(BusiActionParamVO<T> param) throws BusinessException {
		...
		if(!(actionVO.getActioncode().equals(FbmActionConstant.INPUT_SUCCESS)
			...
			return nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000239")/* @res"票据"*/ + param.getBaseinfoVO().getFbmbillno() + nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000272")/* @res"的前一操作必须是入库内部领用单。"*/+ BusiMessageTranslator.translateAction(param);
		}
		return null;
	}
