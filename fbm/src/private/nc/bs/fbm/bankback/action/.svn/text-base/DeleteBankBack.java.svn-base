...
public class DeleteBankBack<K extends HYBillVO,T extends StorageVO> extends ActionBankBack<K, T> {
	...
	protected String doCheck(BusiActionParamVO<T> param) throws BusinessException {
		...
		if(!(actionVO.getActioncode().equals(FbmActionConstant.SAVE)
			...
			return nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000239")/* @res"票据"*/ + param.getBaseinfoVO().getFbmbillno() + nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000246")/* @res"的前一操作必须是保存银行领用单。"*/+ BusiMessageTranslator.translateAction(param);
		}
		return null;
	}
