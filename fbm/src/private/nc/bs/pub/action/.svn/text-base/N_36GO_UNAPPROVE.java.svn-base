...
private String getPk_h(OuterVO[] vos) throws BusinessException {
	if (null == vos || vos.length == 0) {
		throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000369")/* @res"数据错误，找不到与此质押业务对应的物权担保记录，弃审失败"*/);
	}
	if (vos.length > 1) {
		throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("fbmcomm","UPPFBMComm-000370")/* @res"数据错误，与此质押业务对应多条物权担保，弃审失败"*/);
	}
