 ...
 * @author 平台脚本生成
 */
public class N_36GD_APPROVE extends AbstractCompiler2 {
private java.util.Hashtable m_methodReturnHas=new java.util.Hashtable();
private Hashtable m_keyHas=null;
/**
 * N_36GD_APPROVE 构造子注解。
 */
public N_36GD_APPROVE() {
	super();
}
/*
* 备注：平台编写规则类
...
try{
	...
	//内部托管单审核时会报二次对话框，因为在写计划时累计预占用执行数超过核准数，这时会提示是否继续，
    //如果继续，那将VO中的writPlan置为True，但在审核类中重新查询了新的VO，把WritePlan值冲掉了，
	//所以要重新查询之前先把值取出来，重新查询后再把WritePlan值写回去，这样就不会在审核时报二次提示
	//对话框了。
	...
	Object retObj = runClass("nc.bs.trade.comstatus.BillApprove",
			"approveBill", "nc.vo.pub.AggregatedValueObject:36GD", vo,
...
//		ActionParamVO[] params = DefaultParamAdapter.changeInnerKeep2Param(retBillVO,actioncode);
//		FbmActionFactory.getInstance().createActionClass(FbmBusConstant.BILLTYPE_INNERKEEP, actioncode).doAction(params);
		BusiActionFactory.getInstance().createActionClass(FbmBusConstant.BILLTYPE_INNERKEEP, actioncode).doAction(retBillVO, actioncode,false);
